...
(function () {
  ...
  function isConcluded(w) { return !!(w && w.finalizado); }
  ...
  function workCard(w, isOrientador) {
    ...
    return `<div class="card" style="margin-top:10px;">
      ...
      ${isConcluded(w) ? `<div><small><strong>Finalizado em:</strong> ${new Date(w.finalizadoEm || Date.now()).toLocaleDateString()}</small></div>` : ""}
      ...
      <div class="grid grid-2" style="margin-top:8px;">${Object.keys(DATE_LABELS).map(k => `<label>${DATE_LABELS[k]}<input ${isOrientador ? "" : "disabled"} data-date="${k}" data-work="${w.id}" type="date" value="${esc((w.datas && w.datas[k]) || "")}" /></label>`).join("")}</div>
      ${isOrientador ? `<div class="row" style="margin-top:8px;"><select id="st_${w.id}">${STATUS.map(s => `<option ${s === w.status ? "selected" : ""}>${s}</option>`).join("")}</select><input id="pg_${w.id}" type="number" min="0" max="100" value="${Number(w.progresso || 0)}" style="width:90px;" /><button data-save="${w.id}">Salvar</button></div><div class="row" style="margin-top:8px;"><input id="pend_${w.id}" placeholder="Pendências/ajustes" value="${esc(review.pendencias || "")}" style="flex:1;" /><button class="ghost" data-review="${w.id}" data-decision="Aprovado">Aprovar</button><button class="warn" data-review="${w.id}" data-decision="Aprovado com ajustes">Aprovar c/ ajustes</button><button class="alt" data-review="${w.id}" data-decision="Rejeitado">Rejeitar</button>${isConcluded(w) ? `<button class="ghost" data-reopen="${w.id}">Reabrir</button>` : `<button class="warn" data-conclude="${w.id}">Concluir trabalho</button>`}</div>` : ""}
      <h4>Anexos</h4>
      <div>${(w.anexos || []).length ? w.anexos.map(a => a.dataUrl ? `<div><a href="${a.dataUrl}" target="_blank" download="${esc(a.name)}">${esc(a.name)}</a></div>` : `<div>${esc(a.name)} <small>(arquivo antigo sem conteúdo para abrir)</small></div>`).join("") : `<small>Sem anexos.</small>`}</div>
  ...
  function orientadorView() {
    ...
    const categoriasHtml = CATS.map(cat => {
      const group = alunos
        .filter(a => a.categoria === cat)
        .filter(a => !fAluno || a.id === fAluno);

      const body = group.map(al => {
        let ws = state.works.filter(w => w.alunoId === al.id && !isConcluded(w));
        if (fStatus) ws = ws.filter(w => w.status === fStatus);
        if (fTipo) ws = ws.filter(w => w.tipo === fTipo);

        return `<div class="card" style="margin:8px 0;background:#fff;">
          ...
          <div class="row" style="margin-top:6px;"><button class="alt" data-delete-aluno="${al.id}">Apagar aluno</button></div>
          ${ws.length ? ws.map(w => workCard(w, true)).join("") : "<small>Sem trabalhos para este filtro.</small>"}
        </div>`;
      }).join("");
    ...
    const concludedWorks = state.works
      .filter(w => isConcluded(w))
      .filter(w => !fAluno || w.alunoId === fAluno)
      .filter(w => !fStatus || w.status === fStatus)
      .filter(w => !fTipo || w.tipo === fTipo);

    const concludedHtml = concludedWorks.length
      ? concludedWorks.map(w => workCard(w, true)).join("")
      : "<small>Nenhum trabalho concluído para este filtro.</small>";
    ...
    appEl.innerHTML = `<div class="container">
      ...
      <div class="card">
        ...
        <div class="row">
          ...
          <button class="alt" id="clearFilter">Limpar</button>
        </div>
      </div>
  ...
  function alunoView() {
    const ws = state.works.filter(w => w.alunoId === current.id && !isConcluded(w));
  ...
  function bindSharedActions(isOrientador) {
    ...
    if (isOrientador) {
      ...
      appEl.querySelectorAll("button[data-conclude]").forEach(btn => {
        btn.onclick = async () => {
          const wid = btn.getAttribute("data-conclude");
          const w = byId(state.works, wid);
          if (!w) return;
          w.finalizado = true;
          w.finalizadoEm = new Date().toISOString();
          w.status = "Concluído";
          addAudit("Trabalho concluído", `${w.titulo} marcado como concluído`);
          await saveRemoteState();
          render();
        };
      });

      appEl.querySelectorAll("button[data-reopen]").forEach(btn => {
        btn.onclick = async () => {
          const wid = btn.getAttribute("data-reopen");
          const w = byId(state.works, wid);
          if (!w) return;
          w.finalizado = false;
          w.finalizadoEm = null;
          addAudit("Trabalho reaberto", `${w.titulo} reaberto`);
          await saveRemoteState();
          render();
        };
      });
      ...
      appEl.querySelectorAll("button[data-upload]").forEach(btn => {
        btn.onclick = async () => {
          ...
          for (const file of Array.from(f.files)) {
            ...
            w.anexos.push({ name: file.name, size: file.size, type: file.type, dataUrl, ts: Date.now() });
          }
